    ...
    switch (req.query.type) {
      case "information":
        if (req.body.patient) {
          ...
          if (req.body.emergencyperson)
            await prm.postEmergencyPerson(patient.id, req.body.emergencyperson);
          if (req.body.parent)
            await prm.postParent(patient.id, req.body.parent);
        } else {
      ...
      case "form":
        ...
        break;
      case "prescription":
        result = await prm.postPrescription(
          req.query.patientid,
          req.query.dentistid,
          req.body
        );
...
app.get("/app/dentist", async (req, res) => {
  try {
    switch (req.query.type) {
      case "information":
        res.send(await prm.getDentistInformation(req.query));
        break;
      case "login":
        const loginValue = await prm.getDentistInformation(req.query);
        res.send(loginValue);
        break;
      // case 'parent':
      //     res.send(await prm.getParent(req.query));
