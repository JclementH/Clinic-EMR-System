...
const prm = {
    ...
    postMedicalHistory: async (patientID, medicalHistory) => {
        ...
        if(!fsys.existsSync(`${dir}/patient/${patientID}`)) fsys.mkdirSync(`${dir}/patient/${patientID}`);
        if(!fsys.existsSync(`${dir}/patient/${patientID}/p${patientID}_medicalhistory.txt`)) {
            fsys.writeFile(`${dir}/patient/${patientID}/p${patientID}_medicalhistory.txt`, JSON.stringify(medicalHistory), async (err) => {
                if(err) throw err;
    ...
    postForm: async (patientID, dentistID, formType) => {
        ...
        if(!fsys.existsSync(`${dir}/patient`)) fsys.mkdirSync(`${dir}/patient`);
        if(!fsys.existsSync(`${dir}/patient/${patientID}`)) fsys.mkdirSync(`${dir}/patient/${patientID}`);
        if(!fsys.existsSync(`${dir}/patient/${patientID}/forms`)) fsys.mkdirSync(`${dir}/patient/${patientID}/forms`);

        for(i; ; i++) {
            if(!fsys.existsSync(`${dir}/patient/${patientID}/forms/p${patientID}_${formType}_v${i}.pdf`)) {
                const doc = new pdf({size: 'LETTER'});
                doc.pipe(fsys.createWriteStream(`${dir}/patient/${patientID}/forms/p${patientID}_${formType}_v${i}.pdf`));
    ...
    getInformation: async (query) => {
        ...
        if (query.patientid) {
            information = await pool.query(
                `SELECT * FROM patientinformation WHERE id=${query.patientid};`
            );
        } else {
            information = await pool.query(
    ...
    getMedicalHistory: async (query) => {
        ...
        if (query.patientid) {
            ...
            medicalhistory = {"medical history": {"metadata": metadata, "data": JSON.parse(data)}};
        } else {
    ...
    deleteForm: async (query) => {
        if (query.patientid) {
            await pool.query(
                `DELETE FROM form WHERE patientid=${query.patientid};`
            );
        } else {
            return {"error": "missing \'patientid\' query in URL"};
        }
    ...
    deleteImage: async (query) => {
        if (query.patientid) {
            await pool.query(
                `DELETE FROM image WHERE patientid=${query.patientid};`
            );
        } else {
            return {"error": "missing \'patientid\' query in URL"};
        }
